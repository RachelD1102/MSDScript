...
//  ArithemticParser2
//
//  Created by Rachel Di on 3/22/20.
...
LetCont::LetCont(std::string var, PTR(Expr) body, PTR(Env) env, PTR(Cont) rest) {
    this->var = var;
    this->body = body;
    this->env = env;
    this->rest = rest;
...
IfCont::IfCont(PTR(Expr) then_part, PTR(Expr) else_part, PTR(Env) env, PTR(Cont) rest) {
